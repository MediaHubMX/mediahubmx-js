...
const createServerHandler =
  (addons: AddonClass[]): ServerSelftestHandlerFn =>
  async ({ sendResponse }) => {
    sendResponse(200, {
      type: "server",
...
const createAddonHandler =
  ...
  async ({ action, input, sig, request, sendResponse }) => {
    ...
    try {
      ...
      const isSignatureError = [
        "Missing MediaHubMX signature",
        "Invalid MediaHubMX signature",
        "MediaHubMX signature timed out",
